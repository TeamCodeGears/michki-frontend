...
export async function markNotificationsRead(planId) {
  // 빈 POST (토큰만 필요)
  const path = planId
    ? `/plans/${planId}/notifications/read`
    : `/plans/notifications/read`;
  return await fetchJson(path, { method: "POST" });
}

// 알림 목록 조회: 백엔드 엔드포인트가 프로젝트마다 다를 수 있으니
// 안전하게 여러 후보를 시도하고 실패해도 빈 배열을 반환합니다.
export async function getNotifications(planId) {
  const candidates = [];
  if (planId) {
    candidates.push(`/plans/${planId}/notifications`);
    candidates.push(`/plans/${planId}/notifications/list`);
  }
  candidates.push(`/plans/notifications`);
  candidates.push(`/plans/notifications/list`);
  ...
  for (const path of candidates) {
    try {
      ...
      // { notifications: [...] } 같은 래핑 케이스 처리
      if (Array.isArray(data.notifications)) return data.notifications;
    } catch (e) {
      console.error("None Alram", e)
      // 실패하면 다음 후보 시도
    }
  }
  return [];
